...
| **단독 알파벳 키** | y(확인), n(거부), q(닫기), j/k(이동) | IME가 한글 자모로 조합 시작 |

### 영향받는 환경

- 모든 macOS 터미널 (Terminal.app, iTerm2, Alacritty, Kitty, WezTerm)
- 터미널 기반 TUI 앱 (Claude Code, vim, tmux, htop 등)
...
2. **단독 알파벳 키 입력 시** (특정 앱 컨텍스트): 한글 자모 대신 영문 알파벳으로 전송

### 설계 원칙

- **비침투적**: 일반 한글 타이핑에 영향 없음. modifier가 눌렸을 때만 개입
- **단독 키 규칙은 opt-in**: 일반 텍스트 입력과 충돌 가능성이 있으므로 별도 규칙으로 분리
...
### US-2: 한글 상태에서 TUI 앱 단독 키 단축키 사용

> 개발자로서, Claude Code 확인 다이얼로그에서 y/n을 한/영 전환 없이 누르고 싶다.

**수용 조건:**
- 한글 상태에서 y(ㅛ), n(ㅜ), q(ㅂ), j(ㅓ), k(ㅏ) 등이 영문으로 전송됨
- 일반 텍스트 입력 시에는 한글 조합이 정상 동작 (충돌 없음)
- 이 규칙은 별도로 활성화/비활성화 가능
...
### Rule 2: 단독 알파벳 키 한글→영문 매핑 (선택적, 기본 비활성화)

대상 키: y, n, q, j, k, i, r, / (TUI 앱에서 자주 쓰는 단축키)

```
조건: input_source == Korean AND modifier 없음
동작: 해당 영문 알파벳 전송 (한글 자모 대신)
```

> 이 규칙은 일반 텍스트 입력과 충돌할 수 있으므로 기본 비활성화.
> 사용자가 명시적으로 활성화해야 함.

### Rule 3: Meta(Cmd)+키 한글 우회 (선택적)
...
│   ├── standalone-keys.json           # Rule 2: 단독 키 매핑
│   └── meta-keys.json                 # Rule 3: Cmd+키 우회
├── korean-shortcuts.json              # 통합 규칙 (갤러리 배포용)
...
5. Rule 2(단독 키)는 안내만 출력 ("활성화하려면 --with-standalone 옵션")
...
| **단독 키 규칙 충돌** | 일반 텍스트 입력 중 y/n/q 등이 영문으로 전송될 위험 |
...
- [ ] Rule 2(단독 키): 활성화 시 y/n/q/j/k 등 영문으로 전송됨
- [ ] Rule 2: 비활성화 시 일반 한글 입력에 영향 없음
